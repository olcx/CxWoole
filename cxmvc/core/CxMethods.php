 ...
 */
function c($k, $v = null) {
    if (isset(CxSwoole::$config[$k])) {
        if ($v === null) {
            return CxSwoole::$config[$k];
        }
        CxSwoole::$config[$k] = $v;
    }
    return null;
...
function ci($k, $v = null, $file = 'default.inc') {
    if (!isset(CxSwoole::$ini[$file])) {
        if (file_exists(PATH_INI . $file)) {
            CxSwoole::$ini[$file] = unserialize(file_get_contents(PATH_INI . $file));
        }
    }
    if ($v === null) {
        if (isset(CxSwoole::$ini[$file][$k])) {
            return CxSwoole::$ini[$file][$k];
        }
        return null;
    }
    else {
        CxSwoole::$ini[$file][$k] = $v;
        file_put_contents(PATH_INI . $file, serialize(CxSwoole::$ini[$file]));
    }
}
...
function eed($var = null, $var_dump = false) {
    //if (!DEBUG) return;
    //if ($var === null) exit(0);
    e($var, $var_dump);
...
if (!function_exists('ed')) {
    function eds($var = null, $var_dump = false) {
        if (!DEBUG) return;
        if ($var !== null) {
            e($var, $var_dump);
        }
 ...
 * callback
 */
function i($name, $default = false, $filter = 'htmlspecialchars') {
    return CxSwoole::$request;
}
...
function etask($data){
    return CxSwoole::$server->task($data);
}
